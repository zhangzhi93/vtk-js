      ...
      if (xhr.readyState === 4) {
        if (xhr.status === 200 || xhr.status === 0) {
          if (options.compression) {
            resolve(unzlibSync(new Uint8Array(xhr.response)).buffer);
          } else {
            ...
            if (options.compression) {
              if (array.dataType === 'string' || array.dataType === 'JSON') {
                array.buffer = strFromU8(
                  unzlibSync(new Uint8Array(array.buffer))
                );
              } else {
    ...
    xhr.onreadystatechange = (e) => {
      ...
      if (xhr.readyState === 4) {
        ...
        if (xhr.status === 200 || xhr.status === 0) {
          if (options.compression) {
            resolve(
              JSON.parse(strFromU8(unzlibSync(new Uint8Array(xhr.response))))
            );
          } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onreadystatechange = (e) => {
      ...
      if (xhr.readyState === 4) {
        ...
        if (xhr.status === 200 || xhr.status === 0) {
          if (options.compression) {
            resolve(strFromU8(unzlibSync(new Uint8Array(xhr.response))));
          } else {
